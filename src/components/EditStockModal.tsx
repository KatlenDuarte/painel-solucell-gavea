...
import { Package, Plus, Minus, AlertTriangle, BellRing } from "lucide-react";
...
interface EditStockModalProps {
  ...
  onSubmit: (
    ...
    newName: string,
    newPrice: number,
    newMinStock: number // ✅ Adicionado parâmetro aqui
  ) => void;
}
...
export default function EditStockModal({
  ...
  const [newMinStock, setNewMinStock] = useState<string>("0"); // ✅ Novo estado
  ...
  useEffect(() => {
    ...
    setNewMinStock(String(product.minStock)); // ✅ Inicializa com valor atual
  ...
  // Usa o novo estoque mínimo para o cálculo do alerta em tempo real
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedMinStock = Number(newMinStock); // ✅ Converte para número
    ...
    // ✅ Envia todos os campos, incluindo o estoque mínimo
    onSubmit(product.id, newStock, operation, newName, updatedPrice, updatedMinStock);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Editar Produto & Estoque" size="md">
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        {/* MOVIMENTAÇÃO DE ESTOQUE (Mesma lógica de antes) */}
